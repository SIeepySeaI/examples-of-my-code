...
function setEvent(){
  ...
  if (document.getElementById("MapCheckbox").checked) {
    document.getElementById("Workshop").style.zIndex = 0;
  }
  else{
    document.getElementById("Workshop").style.zIndex = -2;
  }
}
function MenuCall() {
...
function OnChangeRange(SliderItem) {
  ...
  if (SliderItemIndex == 0||SliderItemIndex == 1) {
    ...
    document.getElementById(SliderItemIndex + "_InputValue").innerText = (SliderItem.value - 16)/2 + "°С";
  }
...
function ChangeMap(changeMapCB){
  if (changeMapCB.checked) {
    //console.log('Выводим');
    document.getElementById("Workshop").style.zIndex = 0;
  }
  else{
